...
void updateLedBar(int val){
  ...
  for(int i=10; i<=val; i+=10){
      ...
      if(val<30){
        ...
        pixels.setPixelColor(k, pixels.Color(0,2,200));
        pixels.show();    
      }else if(val>=70){
        pixels.setPixelColor(k, pixels.Color(1,150,2));
        pixels.show();
      }
      if(val>=100){
        pixels.setPixelColor(k, pixels.Color(100,100,100));
        pixels.show();
      }
  }
  if(val<90 && !buzzState){
...
void buttonPolling(){
  if(digitalRead(SBUZZ)==ON){
    turnBuzzOff();
    delay(DEBOUNCE);
  }
  if(digitalRead(SCUTOFF)==ON){
    updateCutOff(); 
    delay(DEBOUNCE);
  }  
